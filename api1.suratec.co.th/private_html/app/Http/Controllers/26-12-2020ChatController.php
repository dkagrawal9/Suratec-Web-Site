...
class ChatController extends Controller
{
    ...
    */
    ...
    {
        // $channelData = Channel::where(function ($query)use($id) {
    ...
    //    return $channelData;
        foreach ($channelData as $key => $value) {
            ...
            // if($value->sender_id ==  $id && isset($_REQUEST['mod_type']) && $_REQUEST['mod_type'] == 'mode_employee'){
            ...
            if($value->sender_id ==  $id){
                ...
                if($role && $role->data_role == 'mod_customer'){
                    $finalChannelData[$key]->sender_id = Customer::select('id_customer','id_customer as id','fname','lname')->where('id_customer',$value->sender_id)->first();
                }
                if($role && $role->data_role == 'mod_employee'){
                    $finalChannelData[$key]->sender_id = Employee::select('id_employee','id_employee as id','surname as lname','username as fname')->where('id_employee',$value->sender_id)->first();
                }
                $role_reci = Member::where('id_data_role',$value->reciever_id)->first();
                if($role_reci && $role_reci->data_role == 'mod_customer'){
                    $finalChannelData[$key]->reciever_id = Customer::select('id_customer','id_customer as id','fname','lname')->where('id_customer',$role_reci->id_data_role)->first();
                }
                if($role_reci && $role_reci->data_role == 'mod_employee'){
                    $finalChannelData[$key]->reciever_id = Employee::select('id_employee','id_employee as id','surname as lname','username as fname')->where('id_employee',$role_reci->id_data_role)->first();
                }
            } 
            if($value->reciever_id ==  $id){
                ...
                if($role_reci && $role_reci->data_role == 'mod_customer'){
                    $finalChannelData[$key]->reciever_id = Customer::select('id_customer','id_customer as id','fname','lname')->where('id_customer',$value->reciever_id)->first();
                }
                if($role_reci && $role_reci->data_role == 'mod_employee'){
                    $finalChannelData[$key]->reciever_id = Employee::select('id_employee','id_employee as id','surname as lname','username as fname')->where('id_employee',$value->reciever_id)->first();
                }
            ...
            //     }
            //     $role_reci = Member::where('id_data_role',$value->reciever_id)->first();
            //     if($role_reci->data_role == 'mod_customer'){
            //         $finalChannelData[$key]->reciever_id = Customer::select('id_customer','id_customer as id','fname','lname')->where('id_customer',$role_reci->id_data_role)->first();
            //     }
            //     if($role_reci->data_role == 'mod_employee'){
            //         $finalChannelData[$key]->reciever_id = Employee::select('id_employee','id_employee as id','surname as lname','username as fname')->where('id_employee',$role_reci->id_data_role)->first();
            //     }
            // } 
            // if($value->reciever_id ==  $id){
    ...
    public function messageNotification(Request $request)
    {
        ...
        //return $role;
        if($role->data_role == 'mod_customer'){
            $data = Customer::select('id_customer','id_customer as id','fname','lname')->where('id_customer',$request->sender_id)->first();
            if($data){
                $name = $data->fname ." ".$data->lname; 
            }
        }
        if($role->data_role == 'mod_employee'){
            $data = Employee::select('id_employee','id_employee as id','surname as lname','username as fname')->where('id_employee',$request->sender_id)->first();
