...
require_once '../library/connect.php';
	$str = "SELECT * FROM product_catagory WHERE id_catagory = '".$_POST['id']."' "; //------------------------------------query เพื่อเอา ID
	$query = mysqli_query($objConnect,$str);
	$result = mysqli_fetch_array($query);

//----------------------------------ถ้า level เป็น 1 ให้ลบ ระดับที่ 3 ก่อน 
	if($result['level']==1){
		$str2 = "SELECT * FROM product_catagory WHERE level = '2' AND group_sub = '".$result['id_catagory']."' "; // ดึง id level 2 เพื่เอาไปใส่3
		...
		while($result2 = mysqli_fetch_array($query2)){
			...
			$str_d = "DELETE FROM product_catagory WHERE level = '3' AND group_sub = '".$result2['id_catagory']."' "; // ลบ 3 
		...
		$str3 = "DELETE FROM product_catagory WHERE group_sub = '".$_POST['id']."' ";
		...
		$strSQL = "DELETE FROM product_catagory WHERE id_catagory = '".$_POST["id"]."' ";
		...
		$str_d_img = "SELECT * FROM product_catagory WHERE level = '3' AND group_sub = '".$_POST['id']."' ";
		$query_d_img = mysqli_query($objConnect,$str_d_img);
		...
		$str_sub = "DELETE FROM product_catagory WHERE level = '3' AND group_sub = '".$_POST['id']."' ";
		...
		$str_level2 = "SELECT * FROM product_catagory WHERE id_catagory = '".$_POST['id']."'";
		...
		$str_sub = "DELETE FROM product_catagory WHERE id_catagory = '".$_POST['id']."' ";
		$query_sub = mysqli_query($objConnect,$str_sub);
			if($query_sub){
