...
$data_option = $retData[$z]['action'].'.'.$retData[$z]['duration'];
///////////  right  /////

         ?>
          <option value='<?php echo $retData[$z]['action'].'.'.$retData[$z]['duration']  ?>'
            <?php
            if ($get_time == $data_option) {
                echo "selected";
            }
            ?>
            ><?php echo date('H:i:s',strtotime('+7 hour',strtotime($retData[$z]['action']))).' ('.$retData[$z]['duration'].' sec)' ?></option> 
         <?php
     }
