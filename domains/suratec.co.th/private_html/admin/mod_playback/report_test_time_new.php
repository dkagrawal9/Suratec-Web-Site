...
$result_customer = mysqli_fetch_array($query_customer);
 ...
 $sql_surasole = "SELECT distinct SUBSTRING(`action`,12,20) AS `action` ,`id`, `duration`, `stamp`, `left_sensor1`,
 ...
 `id_customer`, `id_device`, `type`
FROM `surasole` WHERE `id_customer`='$id_customer' AND  `type`='$playback_type'  AND date(action) BETWEEN '$datetimepicker' AND '$datetimepicker' ORDER BY surasole.action ";
...
$option = '<option value="0">เวลา Playback</option>';
$table = '';
...
$option .= " value='".$retData[$z]['action'].'.'.$retData[$z]['duration']."'>".date('H:i:s',strtotime('+7 hour',strtotime($retData[$z]['action']))).' ('.$retData[$z]['duration'].' sec)' ."</option> ";
$table .= '<tr>
<td>'. date('H:i:s',strtotime('+7 hour',strtotime($retData[$z]['action']))).'</td>
...
header('Content-Type: application/json');
echo json_encode(array('status' => '1', 'message' => 'ไม่มี','table'=> $table));    
  }
?>
