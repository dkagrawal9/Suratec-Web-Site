   ...
   <body>
    ...
    //    }
       function function_play() {
         ...
         $.redirect("newReportData2.php?id_customer="+id_customer, {playback_type: playback_type, datetimepicker: datetimepicker, playback_time: playback_time, status_play: '1',upload_csv:files}, "POST", ""); 
       } 
...
for (var i = 1; i < total_length; i++) {
    ...
    if (heelL[i] > 0 && midL[i] > 0 && foreL[i] > 0) {
        count_left2.push(i);
    }

    if (heelL[i] < 10 && midL[i] < 10 && foreL[i] > 50 && column30[i] > 200) {
        count_left3.push(i);
    }
...
for (var i = 1; i < total_length_right; i++) {
    ...
    if (heelR[i] > 0 && midR[i] > 0 && foreR[i] > 0) {
        count_right2.push(i);
    }

    if (heelR[i] < 10 && midR[i] < 10 && foreR[i] > 50 && column29[i] > 200) {
        count_right3.push(i);
    }
...
for (var i = 1; i < (foreL.length - 1); i++) {
    ...
    if (foreL[i] <= foreL[i - 1]) {
        FL_pressure = FL_pressure + parseFloat(foreL[i] * 0.1)
    }

    if (foreL[i] > foreL[i - 1]) {
        FL_pressure = FL_pressure + parseFloat((foreL[i - 1] * 0.1))
    }

    if (midL[i] <= midL[i - 1]) {
        ML_pressure = ML_pressure + parseFloat(midL[i] * 0.1)
    }

    if (midL[i] > midL[i - 1]) {
        ML_pressure = ML_pressure + parseFloat(midL[i - 1] * 0.1)
    }

    if (heelL[i] <= heelL[i - 1]) {
        HL_pressure = HL_pressure + parseFloat(heelL[i] * 0.1)
    }

    if (heelL[i] > heelL[i - 1]) {
        HL_pressure = ML_pressure + parseFloat(heelL[i - 1] * 0.1)
    }

    if (foreR[i] <= foreR[i - 1]) {
        FR_pressure = FR_pressure + parseFloat(foreR[i] * 0.1)
    }

    if (foreR[i] > foreR[i - 1]) {
        FR_pressure = FR_pressure + parseFloat(foreR[i - 1] * 0.1)
    }

    if (midR[i] <= midR[i - 1]) {
        MR_pressure = MR_pressure + parseFloat(midR[i] * 0.1)
    }

    if (midR[i] > midR[i - 1]) {
        MR_pressure = MR_pressure + parseFloat(midR[i - 1] * 0.1)
    }

    if (heelR[i] <= heelR[i - 1]) {
        HR_pressure = HR_pressure + parseFloat(heelR[i] * 0.1)
    }

    if (heelR[i] > heelR[i - 1]) {
        HR_pressure = HR_pressure + parseFloat(heelR[i - 1] * 0.1)
    }
