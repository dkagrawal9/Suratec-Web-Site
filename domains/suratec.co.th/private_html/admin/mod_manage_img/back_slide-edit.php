<?php 
	...
	if(empty($_FILES['image_slide']['name'])){
		...
		$strSQL = "SELECT name_image FROM images WHERE id_image = '".$_POST["id_image"]."'";
		$objQuery = mysqli_query($objConnect,$strSQL);
		...
		if(unlink("../../uploads/mod_image/".$file)){
			...
			$strSQL .= " id_category = '".$_POST['image_category_edit']."' ";
			$strSQL .= " ,name_image = '".$newname."' ";
			$strSQL .= " ,date_action = '".$date."' ";
			$strSQL .= " ,size = '".$_FILES["image_slide"]["size"]."' ";
			$strSQL .= "WHERE id_image= '".$_POST['id_image']."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
			if ($objQuery) {
