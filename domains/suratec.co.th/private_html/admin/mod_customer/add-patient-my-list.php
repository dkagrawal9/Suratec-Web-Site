<?php
	...
	try {
		...
		if (!empty($_POST)) {
			...
			$delQuery = "UPDATE mod_customer SET `assigned_dr`='".$member->id_data_role."' WHERE id_customer='$id_customer'";

			if ($objConnect->query($delQuery) === TRUE) {
				$response = ['status'=> 200,'message'=> 'Patient added to your list successfully'];
			} else {
