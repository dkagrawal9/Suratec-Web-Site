    ...
    <script type="text/javascript">
      ...
      $(document).ready(function () {
          ...
          "columns": [{
            ...
            /*{ "data": "telephone" },*/
            {
              "data": function (data, type, dataToSet) {
                let action = '';
                action = "<button style='" + input_read +
                  "' class='btn btn-primary btn-sm show-cus'><i class='fa fa-fw fa-eye'></i></button>" +
                  "<button style='" + button_edit +
                  "' class='btn btn-warning btn-sm edit-cus' ><i class='fa fa-fw fa-pencil'></i></button>" +
                  "<button style='" + button_del +
                  "' class='btn btn-danger btn-sm del-cus'><i class='fa fa-fw fa-trash'></i></button>" +
                  "<button style='" + input_read +
                  "' class='btn btn-info btn-sm playback-cus'>playback</button>" +
                  "<button class='btn btn-primary btn-sm cust-chat'><i class='fa fa-comments'></i></button>";
                  if (data.assigned_dr) {
                      action += "<button class='btn btn-success btn-sm remove-patient' title='Remove patient to my list'><i class='fa fa-minus-square' aria-hidden='true'></i></button>";
                    }
                    console.log(data)
                    return action;
                return action;
        ...
        $(document).on('click','.remove-patient',function(){
                ...
                $.ajax({
                    method: "POST",
                    url: "remove-patient-my-list.php",
                    data: {id_customer}
                }).done(function(res) {
                    if (res.status === 200) {
                        table.ajax.reload();
