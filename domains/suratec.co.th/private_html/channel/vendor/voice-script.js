...
function handleEvents (rtc) {
  ...
  rtc.client.on("peer-leave", function (evt) {
    ...
    Toast.info(`${userName} has left the call`);
  ...
  rtc.client.on("stream-subscribed", function (evt) {
    ...
    Toast.notice(`${userName} has joined the call`);
  ...
  rtc.client.init(option.appID, function () {
    ...
    **/
    rtc.client.join(option.token ? option.token : null, option.channel, option.uid ? +option.uid : null, function (uid) {
      // Toast.notice(" : " + option.channel + " success, uid: " + uid)
      // Toast.notice(`${userName} has joined the call 1`)
      Toast.info(`You have joined the call`);
      ...
      rtc.localStream.init(function () {
        ...
        // Update the appointment so patient can join the call
        if (userType === 'doctor') {
          updateCallStatus();
        }
      }, function (err)  {
...
function startVideoCall(){
  const options = {
    mode: "live",
    codec: "h264",
    appID: '427de33581664f158e837109a11077db',
