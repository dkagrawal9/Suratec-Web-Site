...
if(isset($row)){
    if ($query) {
        echo  json_encode(array('code' => 200, 'message' => $ggID));
        } else {
        echo  json_encode(array('code' => 404, 'message' => $sql_type));
        }
}else{

    $sql = "INSERT INTO mod_customer  (id_customer, fname, lname, birthday,img_path,email,id_google) 
    VALUES ('".$id_cust."','".$first_name."','".$last_name."','".$birthday."','".$pic."','".$ggEmail."','".$ggID."')";
    print $sql;
$query = mysqli_query($objConnect, $sql);
...
//	echo $sql_io;
	...
	if($query_order){
		echo json_encode(array('status' => '1','message'=> $sql_io));
	}else{
...
$date = date("Y-m-d H:i:sa");
		...
		$str_a = "INSERT INTO mod_customer_address (address,district,amphur,province,postalcode,status,create_datetime,update_datetime,id_customer,id_address)
		...
		$str_a2 = "INSERT INTO mod_customer_address (address,district,amphur,province,postalcode,status,create_datetime,update_datetime,id_customer,id_address)
...
function CHACK_NEW_PASS(){
  ...
  date_default_timezone_set("Asia/Bangkok");
	...
	if(isset($_POST["username"]) && isset($_POST["password"]) ){
		...
		if(!$result) {
			echo json_encode(array('status' => '0','message'=> 'Error login data!'+$str));
		}else{
...
$num = mysqli_num_rows($query);
	...
	{
		echo json_encode(array('status' => 'ผิดพลาด'));
	}
/*if ($count > 0 || $count == 0) {
    ...
    $duration = $result['duration'];
                    ...
                    $results = array('duration' => $duration,'left_sensor'=> $left_sensor,'right_sensor'=> $right_sensor,'number_all' =>$num);
