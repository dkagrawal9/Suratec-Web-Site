<?php
	...
	try {
		...
		if (!empty($_POST)) {
			...
			$delQuery = "UPDATE appointments SET `status`=2 WHERE id='$id' AND id_customer='$id_customer'";

			if ($objConnect->query($delQuery) === TRUE) {
				echo json_encode(['status'=> 200,'message'=> 'Your appointment is cancelled successfully']);exit;
			} else {
