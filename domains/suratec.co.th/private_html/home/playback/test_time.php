...
FROM `surasole` WHERE `id_customer`='$id_customer'   AND date(action) BETWEEN '$datetimepicker' AND '$datetimepicker'  ORDER BY `id` ASC";
                ...
                if ($i != 0) {
                   // echo $data[$i]['duration'] - $data[$i - 1]['duration'];
                    if ($data[$i]['duration'] - $data[$i - 1]['duration'] >= 0) {
                        $lsensor = [$data[$i]['left_sensor1'], $data[$i]['left_sensor2'], $data[$i]['left_sensor3'], $data[$i]['left_sensor4'], $data[$i]['left_sensor5'],$data[$i]['duration']+1];
                        $rsensor = [$data[$i]['right_sensor1'], $data[$i]['right_sensor2'], $data[$i]['right_sensor3'], $data[$i]['right_sensor4'], $data[$i]['right_sensor5'],$data[$i]['duration']+1];
...
///////////  right  /////
$duration = intval($retData[$z]['duration'])+1;
         ?>
          <option  value='<?php echo $retData_left_text.'/'.$retData_right_text  ?>'
            ...
            ?>
            ><?php echo $retData[$z]['action'].' ('.$duration.' sec)' ?></option> 
         <?php
     }
?>