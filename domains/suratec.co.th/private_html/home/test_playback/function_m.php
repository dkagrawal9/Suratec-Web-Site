...
date_default_timezone_set("Asia/Bangkok");
	...
	$id = explode(',',$id);
     ...
     for($j=$i; count($id) > $i;$i++){
	  ...
	  $sql = 'SELECT * FROM mod_customer_address WHERE id_customer = "'.$id_customer.'"';
	  $query = mysqli_query($objConnect,$sql);
	  $num = mysqli_num_rows($query);
	  if($num>0){
	  ...
	  $sql_se_c = 'SELECT * FROM mod_customer_address WHERE id_customer = "'.$id_customer.'" ';
      ...
      VALUES('BL$refno','$date','$id_customer','".$result_se_c['id_address']."','$address $district $amphur $province $postalcode $telephone','1','2','Wait_shipping','$total','$refno')";
      $query_order = mysqli_multi_query($objConnect, $str_order);
	   ...
	   $sql_io = "INSERT INTO mod_order_item (`id_order`, `id_product`, `qty`, `price`, `subtotal`) 
               SELECT 'BL$refno' AS id_order ,
			   product.id_product ,
			   1 as qty,
			   product.tmp_price,
			   (1* product.tmp_price) AS subtotal
               FROM product LEFT JOIN product_image
               ON product.id_product = product_image.id_product
               where product.id_product in($data)";
